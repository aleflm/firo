...
UniValue sendtoanyaddress(const JSONRPCRequest& request)
{
    ...
    // Simple balance checks - check what's available for each type
    if (!transparentRecipients.empty()) {
        CAmount transparentBalance = pwallet->GetBalance();
        if (transparentBalance < totalTransparentAmount) {
    ...
    // Handle transparent transactions - use regular transparent coins only
    if (!transparentRecipients.empty()) {
        if (pwallet->GetBroadcastTransactions() && !g_connman) {
            throw JSONRPCError(RPC_CLIENT_P2P_DISABLED, "Error: Peer-to-peer functionality missing or disabled");
        }
